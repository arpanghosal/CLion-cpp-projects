	...
	for (rod_iterator=RodCrateConn->rodBocs().begin();rod_iterator!=RodCrateConn->rodBocs().end(); rod_iterator++){
	  ...
	  for (int ob_number=0; ob_number<=3; ob_number++){
		...
		if (!mod_active)  {
		  ...
		  Map.insert(pair<string,string>(rod_name,mod_name));
		}
		if (!mod_enabled)  {
		  ...
		  nDis_pay++;
            ...
            //print out the map of Rod and Corresponding module
            multimap<string,string>::iterator itr;
            cout<<"Diabled modules in disabled rod " << rod_name <<" are : \n";
            cout<<"\tROD\tMODULE\n";
